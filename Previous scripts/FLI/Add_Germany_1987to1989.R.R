...
#foxrabies <-readOGR("C:/Users/Micaela/Dropbox/Fox Rabies_MD/Scripts/output/foxrabies_final1.shp", "foxrabies_final1") # file created in QGIS by Katie.
foxrabies <-readOGR("C:/Users/Micaela/Dropbox/Fox Rabies_MD/Scripts/output/foxrabies_final_full.shp", 
                    "foxrabies_final_full") # file created in QGIS by Katie.
names(foxrabies)
#[1] "YEAR"    "QUARTER" "COUNTRY" "REGION"  "GADM_L3" "SPECIES" "NUMBER"  "Qs"      "Ss" 

# Set projection, coordinates are ok
# year and species are fine
# Find out real GADM
# Find out region
# Find out where is stated the number of cases
# Convert Qs and Ss using datum
...
# Plot to check if projection is working
plot(foxrabies)
plot(germany_subset1, add=TRUE, col="red")
...
######
germany_add.df <- as.data.frame(germany_add)

year.i <-sort((unique(germany_add.df$YEAR)), decreasing = FALSE)
year.i  <- c(1987:1990)

map <- get_map(location = c(lon = 11, lat = 51),     # select Europe
               zoom = 6,                # 5 to set a higher scale.
...
for(i in seq_along(year.i)){
  ...
  ger.ggplot <- germany_add.df[which(germany_add.df$YEAR==year.i[i]),] 
  # Create the map with fox rabies data
  
  maps <- ggmap(map) + xlab("Longitude") + ylab("Latitude") +
    geom_point(data = ger.ggplot,
